...
void sort_names(string name)//This function can write names into a new file in sorted sizes order.
{
	...
	while ((!names.eof()) and buffer->length() <= MAX_LENGTH - 1)
	{
		names >> buffer[index];
		index++;
	}
...
void order_name(string* names,int size)
...
int get_size(string* names)//Judge the effective size of a string array.
{
	int size,index=0;
